<template>
  ...
  <div class="row_nw_fs_center lat_container" @wheel="mouseWheelHandle">
    ...
    <div v-else-if="inputMode === 'HUMAN'" class="row_nw_fs_center wh_100p_100p">
      ...
      />
      <span class="row_nw_center_center lat_label" @click="snHumanHandle">{{ latObj.isSouth ? "S" : "N" }}</span>
    </div>
    <div v-else-if="inputMode === 'DMS'" class="row_nw_fs_center wh_100p_100p">
      <span class="row_nw_center_center lat_label" @click="snDmsHandle">{{ latObj.isSouth ? "S:" : "N:" }}</span>
      <input
...
function sliderHandle() {
  if (typeof latObj.latitude === "string") {
    latObj.latitude = parseFloat(latObj.latitude);
  }
