<template>
  ...
  <div class="row_nw_fs_center lng_container" @wheel="mouseWheelHandle">
    ...
    <div v-else-if="inputMode === 'HUMAN'" class="row_nw_fs_center wh_100p_100p">
      ...
      />
      <span class="row_nw_center_center lng_label" @click="weHumanHandle">{{ lngObj.isWest ? "W" : "E" }}</span>
    </div>
    <div v-else-if="inputMode === 'DMS'" class="row_nw_fs_center wh_100p_100p">
      <span class="row_nw_center_center lng_label" @click="weDmsHandle">{{ lngObj.isWest ? "W:" : "E:" }}</span>
      <input
...
function sliderHandle() {
  console.log("sliderHandle", typeof lngObj.longitude);
  if (typeof lngObj.longitude === "string") {
    lngObj.longitude = parseFloat(lngObj.longitude);
  }
...
.lng_container:focus-within {
  border: 1px solid var(--c-064-158-255-100);
}
...
.lng_dms {
  ...
  /* border-bottom: 1px solid var(--c-026-030-039-040); */
}
...
.lng_range_input {
  ...
  appearance: auto;
  accent-color: var(--c-074-074-074-100);
}
