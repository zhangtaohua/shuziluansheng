...
export default class CsGeojsonLayers {
        ...
        if (entity && entity.properties) {
          ...
          if (customMetaNew && customMetaNew[customMeta].id == options.id) {
            ...
            if (!options.popup?.position) {
              popupOpt.position = [lngLatHeight.longitude, lngLatHeight.latitude, 0];
            }
  ...
  public createLayer(
    ...
    // console.log("fill", options.id, geojsonOpt, fill, Cesium.GeoJsonDataSource.fill);
    ...
    if (this.csBaseHandle) {
      ...
      if (layerObj) {
        layerObj.geojsonPromsie
          ...
          .catch((err) => {
            console.log("Geojson add Failed!", err);
          });
        return true;
      } else {
        return false;
  ...
  public clearLayer() {
    if (this.csBaseHandle && this.__layers.size) {
      this.__layers.forEach((layerObj: any) => {
        ...
        this.dataSources.remove(layerObj.dataSource, true);
  ...
  public setSplitLayer(options: GeojsonTopoJSONOptions, direction: string | null = null) {
      ...
      if (layerObj) {
        ...
        if (direction) {
          if (direction.toUpperCase().includes("L")) {
            for (let i = 0; i < entities.length; i++) {
              const entity = entities[i];
              entity.splitDirection = Cesium.SplitDirection.LEFT;
            }
            // this.dataSources.splitDirection = Cesium.SplitDirection.LEFT;
          } else {
            for (let i = 0; i < entities.length; i++) {
              const entity = entities[i];
              entity.splitDirection = Cesium.SplitDirection.RIGHT;
            }
            // this.dataSources.splitDirection = Cesium.SplitDirection.RIGHT;
          }
        } else {
          for (let i = 0; i < entities.length; i++) {
            const entity = entities[i];
            entity.splitDirection = Cesium.SplitDirection.NONE;
          }
          // this.dataSources.splitDirection = Cesium.SplitDirection.NONE;
        }
        return true;
      } else {
        return false;
