...
export default class CsVueNodePopup {
  ...
  public createLayer(options: VueNodeOptions) {
    ...
    const postRenderFunc = this.postRender(options);
  ...
  public updatePosition(options, positionOrg) {
    const layerObj = this.__layers.get(this.__Id(options.id));
    if (layerObj) {
      const position = Cesium.Cartesian3.fromDegrees(positionOrg[0], positionOrg[1], positionOrg[2]);
      const height = positionOrg[2];
      layerObj.position = position;
      layerObj.height = height;
    }
  }

  public updateVnodeData(options, vnodeData) {
    const layerObj = this.__layers.get(this.__Id(options.id));
    if (layerObj) {
      layerObj.options.vNodeData = vnodeData;
    }
  }
