...
export default class CsVueNodePopup {
  ...
  public mapRootDom: HTMLElement | null;
  ...
  constructor(mapBaseIns: CesiumBase) {
    ...
    // console.log("this.viewer", this.viewer, this.mapDomContainner);
    this.mapRootDom = document.getElementById(mapBaseIns.container);
  ...
  public postRender(vDom: any, position: any, heightOrg: any, offset: any) {
    return () => {
      ...
      const distance = Cesium.Cartesian3.distance(camerPosition, position);
      // console.log("nnd", distance, height);
      // if (!(distance > height) && this.viewer.camera.positionCartographic.height < 50000000) {
  ...
  public createLayer(options: VueNodeOptions) {
    ...
    const postRenderFunc = this.postRender(vDom, position, height, options.offset);
  ...
  public clearLayer() {
    if (this.csBaseHandle && this.__layers.size) {
      ...
      this.__DomMap.forEach((oldParentDom: any) => {
        if (oldParentDom) {
          const oldGrandFather = oldParentDom?.parentNode;
          oldGrandFather?.removeChild(oldParentDom);
        }
  ...
  public showPopupByID = (id: string) => {
    if (this.csBaseHandle) {
      ...
      if (layerObj) {
        if (layerObj.isShowed == false) {
          this.addPostRender(layerObj.postRenderFunc);
          layerObj.isShowed = true;
        }
        return true;
      } else {
        return false;
  ...
  public hiddenAllPopup() {
    this.__layers.forEach((layerObj: any) => {
      if (layerObj.isShowed == true) {
        layerObj.vDom.style.display = "none";
        this.removePostRender(layerObj.postRenderFunc);
        layerObj.isShowed = false;
      }
