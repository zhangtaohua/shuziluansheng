...
export default class CsBillboardLayers {
    ...
    return (event: any) => {
      ...
      if (pick != undefined) {
        ...
        if (entity && entity.properties) {
          ...
          if (entityId == options.id || entityId.getValue() == options.id) {
  ...
  public setPosition(options: BillboardsOptions, position: any) {
    return this.setPositionByID(options.id, position);
  }

  public setPositionByID(id: string, position: any) {
    if (this.csBaseHandle) {
      const layerObj = this.__layers.get(this.__Id(id));
      if (layerObj) {
        ...
        layerObj.entity.position.setValue(position);
        return true;
      } else {
        return false;
      }
