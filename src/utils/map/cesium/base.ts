...
import type { CesiumBasicOptions, PskyboxSource } from "./baseTypes";
...
export default class CesiumBase {
  ...
  public eventCanvasHandler: any = null;
  ...
  constructor(target: string, options: CesiumBasicOptions) {
    ...
    if (
      ...
      //判断是否支持图像渲染像素化处理
      this.viewer.resolutionScale = this.pixelRatio;
    }
    ...
    this.eventCanvasHandler = new Cesium.ScreenSpaceEventHandler(this.viewer.scene.canvas);
    ...
    this.__domRO = new ResizeObserver((entries) => {
      this.onWindowResize();
    });

    // 观察一个或多个元素
    this.__domRO.observe(document.getElementById(target));
  ...
  public getResolutionScale = (parentDom: any) => {
    ...
    // const left = scene.camera.getPickRay(new Cesium.Cartesian2(offsetLeft + 4, height / 2 + offsetTop - 4));
    // const right = scene.camera.getPickRay(new Cesium.Cartesian2(offsetLeft + width - 4, height / 2 + offsetTop - 4));

    const left = scene.camera.getPickRay(new Cesium.Cartesian2(4, height / 2));
    const right = scene.camera.getPickRay(new Cesium.Cartesian2(width - 4, height / 2));

    const globe = scene.globe;
    const leftPosition = globe.pick(left, scene);
    const rightPosition = globe.pick(right, scene);
    // console.log("getResolutionScale", offsetLeft, offsetTop, width, height);
    // console.log("getResolutionScale2", left, right, leftPosition, rightPosition);
    if (!leftPosition) {
      return null;
    }
    if (!rightPosition) {
    ...
    const resolution = pixelDistance / (width - 8);
    // let scale = (resolution * 96) / 0.0254;
    const scale = resolution * 96;
    return scale;
  };

  public getResolutionScaleV2 = (parentDom: any) => {
    const scene = this.viewer.scene;
    ...
    // Get left and right points on the canvas
    const leftPick = new Cesium.Cartesian2(4, canvasHeight / 2); // Left edge
    const rightPick = new Cesium.Cartesian2(canvasWidth - 4, canvasHeight / 2); // Right edge

    // Use camera.pickEllipsoid to get positions in world space
    const leftPosition = this.viewer.camera.pickEllipsoid(leftPick, ellipsoid);
    const rightPosition = this.viewer.camera.pickEllipsoid(rightPick, ellipsoid);
    // console.log("getResolutionScale", offsetLeft, offsetTop, width, height);
    // console.log("getResolutionScale2", left, right, leftPosition, rightPosition);
    if (!leftPosition) {
      return null;
    ...
    //根据屏幕左侧到右侧的距离测算resolution
    const pixelDistance = Cesium.Cartesian3.distance(leftPosition, rightPosition);
    const resolution = pixelDistance / (canvasWidth - 8);
    const scale = (resolution * 96) / 0.0254;
    return scale;
  };

  public addGetScaleEvent() {
    this.viewer.scene.postRender.addEventListener(this.getResolutionScale);
  }

  public removeGetScaleEvent() {
    this.viewer.scene.postRender.removeEventListener(this.getResolutionScale);
  }
  ...
  public heightToZoom(height: number) {
    ...
    const C = 91610.74;
    const D = -40467.74;
    const level = Math.round(D + (A - D) / (1 + Math.pow(Number(height) / C, B))) + 1;
  ...
  public zoomInOut(isZoomin = true, duration = 2) {
    ...
    const height = this.camera.positionCartographic.height;
    let currentZoom: number = this.getZoomLevel(height);
    if (currentZoom) {
      if (isZoomin) {
        currentZoom += 1;
  ...
  public setCameraView(options: any) {
    // console.log("setCameraView", options);
    const cartesianPosition = Cesium.Cartesian3.fromDegrees(options.longitude, options.latitude, options.height);
  ...
  public setCameraTargetPosOrientation(options: any) {
    if (this.camera) {
      const target = Cesium.Cartesian3.fromDegrees(options.longitude, options.latitude, options.targetHeight);
      const pr = new Cesium.HeadingPitchRange(options.heading, options.pitch, options.height);
      this.camera.lookAt(target, pr);
      this.camera.setView({
        orientation: {
          heading: options.heading,
          pitch: options.pitch,
  ...
  public fitToExtent(extent: any, duration = 2) {
    if (this.camera && extent.length >= 4) {
      this.camera.flyTo({
  ...
  public fitToExtentCenter(extent: any, duration = 2) {
  ...
  public fitToExtentWithHeight(extent: any, height: any, duration = 2) {
    const center = getCenterFromExtent(extent);
    if (center?.length) {
      this.flyToLngLatHeight(center[0], center[1], height, duration);
    }
  }
